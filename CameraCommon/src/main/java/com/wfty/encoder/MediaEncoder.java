...
import com.wfty.core.Packager;
import com.wfty.rtmp.RESFlvData;
import com.wfty.tools.ByteArrayTools;
import com.wfty.tools.Util;
...
//		mMediaCodec.signalEndOfInputStream();	// API >= 18
	...
	protected void encode(final byte[] buffer, final int length, final long presentationTimeUs) {
   	if (DEBUG) Log.v(TAG, "encode1:buffer=" + Util.bytesToHexString(buffer));
...
//	            if (DEBUG) Log.v(TAG, "encode:queueInputBuffer");
	...
	protected void encode(final ByteBuffer buffer, final int length, final long presentationTimeUs) {
   	...
   	if (DEBUG) Log.v(TAG, "encode2:buffer=");
    	if (!mIsCapturing) return;
    	int ix = 0, sz;
        final ByteBuffer[] inputBuffers = mMediaCodec.getInputBuffers();
        while (mIsCapturing && ix < length) {
...
LOOP:	while (mIsCapturing) {
	...
	private void sendAVCDecoderConfigurationRecord(long tms, MediaFormat format) {
		...
		byte[] AVCDecoderConfigurationRecord = generateAVCDecoderConfigurationRecord(format);
		Log.e("hjs","AVCDecoderConfigurationRecord="+AVCDecoderConfigurationRecord.length);
